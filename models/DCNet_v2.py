...
def DCNet_v2(config, weights=None):
    """create UNet with given config"""
    model = DCNetModel(config, encoder_arch='resnet50', pretrained=config.network.pretrained)
...
class DCNetModel(nn.Module):
    def __init__(self, config, encoder_arch='resnet50', pretrained=True):
        ...
        """
        super(DCNetModel, self).__init__()
        self.config = config
        ...
        # !!!!!!!!!! remember to adjust !!!!!!!!!!!!!!!!
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        self.channel1 = ChannelConV(512, 256)
        self.channel2 = ChannelConV(256, 128)
        self.channel3 = ChannelConV(64, 64)

        # bottleneck -> BCP module
        self.bcp = BCP(1024, 512)

        self.myUpsample = myUpsample(512, 256, 256)
        ...
        # decoder -> BRU module
        self.bru1 = BRU(256, 256, 128, 128)
        self.bru2 = BRU(128, 128, 64, 64)
        self.bru3 = BRU(64, 64, 64, 64)
    ...
    def forward(self, input0):
        ...
        middle = self.myUpsample(middle)
        
        conv3 = self.channel1(conv3)
        conv2 = self.channel2(conv2)
        conv1 = self.channel3(conv1)

        deconv3 = self.bru1(middle, conv3)
        deconv2 = self.bru2(deconv3, conv2)
        deconv1 = self.bru3(deconv2, conv1)
...
class myUpsample(nn.Module):
    def __init__(self, input_nc, output_nc, groups):
        super(myUpsample, self).__init__()
        self.deconv = nn.ConvTranspose2d(input_nc, output_nc, kernel_size=4, stride=2, padding=0, output_padding=0,
                                         groups=groups, bias=False)
    def forward(self, x):
